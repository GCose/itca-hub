...
const useResourceViewer = ({ resourceId }: UseResourceViewerProps) => {
    ...
    const fetchResource = async () => {
      ...
      try {
        ...
        const foundResourceEntry = Object.entries(metadataMap).find(
          ([fileName, data]) =>
            (data as Record<string, unknown>).id === resourceId ||
            fileName === resourceId
        );
        ...
        const completeResource: Resource = {
          id:
            ((resourceData as Record<string, unknown>).id as string) ||
            resourceId,
          fileName,
          title:
          ...
          type:
            ((resourceData as Record<string, unknown>).type as string) ||
            fileExtension,
          category:
            ((resourceData as Record<string, unknown>).category as string) ||
            "Uncategorized",
          dateUploaded:
            ((resourceData as Record<string, unknown>)
              .dateUploaded as string) ||
            new Date().toISOString().split("T")[0],
          fileSize:
            ((resourceData as Record<string, unknown>).fileSize as string) ||
            "Unknown size",
          downloads:
            ((resourceData as Record<string, unknown>).downloads as number) ||
            0,
          viewCount:
            ((resourceData as Record<string, unknown>).viewCount as number) ||
            0,
          fileUrl: (resourceData as Record<string, unknown>).fileUrl as string,
          visibility: (((resourceData as Record<string, unknown>)
            .visibility as string) || "all") as "all" | "admin",
          featuredOnLanding:
            ((resourceData as Record<string, unknown>)
              .featuredOnLanding as boolean) || false,
          department:
            ((resourceData as Record<string, unknown>).department as string) ||
            "computer-science",
        };
  ...
  const getFileType = () => {
    ...
    if (["jpg", "jpeg", "png", "gif", "webp"].includes(extension || ""))
      return "image";
    if (
      ["mp4", "webm", "ogg", "mkv", "avi", "mov", "wmv", "3gp", "flv"].includes(
        extension || ""
      )
    )
      return "video";
